...
Board::Board(Markup::Node& document) {
  ...
  information.type = document["board"].text();

  if(auto memory = Emulator::Game::Memory{document["board/memory(type=ROM,content=Program)"]}) {
  ...
  if(auto memory = Emulator::Game::Memory{document["board/memory(type=RAM,content=Save)"]}) {
  ...
  if(auto memory = Emulator::Game::Memory{document["board/memory(type=ROM,content=Character)"]}) {
  ...
  if(auto memory = Emulator::Game::Memory{document["board/memory(type=RAM,content=Character)"]}) {
...
auto Board::load(string manifest) -> Board* {
  auto gameDocument = BML::unserialize(manifest);
